...
local function _1_()
  local function create_keymaps(bufid)
    ...
    vim.keymap.set("n", "[e", string.format("<Cmd>call search('%s', 'bw')<CR>", p), {buffer = bufid, silent = true, desc = "[hurl] Goto prev entry"})
    vim.keymap.set("n", "]e", string.format("<Cmd>call search('%s', 'w')<CR>", p), {buffer = bufid, silent = true, desc = "[hurl] Goto next entry"})
...
local function _3_()
  ...
  local function _8_(ev)
    ...
    return create_keymaps(ev.buf)
  end
  return vim.api.nvim_create_autocmd("FileType", {pattern = {"http", "rest"}, callback = _8_})
end
